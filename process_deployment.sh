...
aws s3 cp appspec.yaml s3://codehub-dev-data
aws deploy wait deployment-successful --deployment-id $(aws deploy create-deployment --cli-input-json file://codehub-search-create-deployment.json --region us-east-1 | jq -r ".deploymentId")
aws s3 sync s3://codehub-updated-data .
ls -l
